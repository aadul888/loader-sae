        ...
        function clearLog() {
            document.getElementById('log-container').innerHTML = '<div class="text-muted">Log dibersihkan...</div>';
        }
            ...
            fetch('index.php?action=sync_progress')
                ...
                .then(data => {
                    ...
                    if (p.stage === 'fetching' || p.stage === 'fetched') {
                        ...
                        if (p.stage === 'fetched') pct = Math.round((p.ep_index / p.total_endpoints) * 80);
                    ...
                    if (detailText) {
                        if (p.stage === 'fetching' || p.stage === 'fetched') {
                            detailText.textContent = 'Endpoint ' + p.ep_index + ' dari ' + p.total_endpoints + ' (' + p.rows_done + ' data)';
                        } else if (p.stage === 'pushing') {
                            detailText.textContent = 'Mengirim ' + p.rows_total + ' data ke SAE...';
                        } else if (p.stage === 'done') {
                    ...
                    if (p.stage === 'done' || p.stage === 'error') {
                        setTimeout(() => { clearSyncInProgress(); }, 2000);
                    }
                })
                .catch(() => {});
        }
        ...
        function clearSyncInProgress() {
            ...
            setTimeout(() => {
                ...
                if (pb) { pb.style.width = '0%'; pb.classList.remove('bg-danger'); }
            }, 3000);
        }
        ...
        function testDapodikConnection() {
            ...
            showConfigResult('loading', 'Memverifikasi token Dapodik...');
            ...
            fetch('index.php?action=test_dapodik', {
                ...
                .then(data => {
                    if (data.success) {
                        showConfigResult(true, 'Koneksi Dapodik berhasil');
                        setTimeout(() => location.reload(), 2000);
                    } else {
                        showConfigResult(false, 'Koneksi Dapodik gagal: ' + (data.message || ''));
                    }
                })
                .catch(error => {
                ...
                .then(data => {
                    ...
                    try {
                        if (data && data.dapodik) {
                            ...
                            if (dBadge) {
                                dBadge.className = 'mini-badge ' + (data.dapodik.status ? 'success' : 'error');
                                dBadge.textContent = data.dapodik.status ? 'Terhubung' : 'Belum siap';
                            }
                            if (dEndpoint && data.dapodik.endpoint) dEndpoint.textContent = data.dapodik.endpoint;
                        }
                        if (data && data.sae) {
                            const sBadge = document.getElementById('sae-status-badge');
                            const sEndpoint = document.getElementById('sae-endpoint');
                            if (sBadge) {
                                sBadge.className = 'mini-badge ' + (data.sae.status ? 'success' : 'error');
                                sBadge.textContent = data.sae.status ? 'Terhubung' : 'Belum siap';
                            }
                            if (sEndpoint && data.sae.endpoint) sEndpoint.textContent = data.sae.endpoint;

                            // If SAE became disconnected, reload so server-side UI shows config form
                            if (!data.sae.status) {
                                console.warn('SAE reported disconnected — reloading to show config form');
